...
`Utils.adjustCareerOverflow(container)` がプレビュー表示時・PDF生成時・入力画面での編集時に呼び出される。
...
- 移動後、溢れが発生するか実測で判定（DOMのBoundingClientRectを使用）
- 溢れなし → career-page2を削除（統合成功）
- 溢れあり → page2に内容を戻す（独立ページとして維持）
- 残余スペースの推定計算は行わず、実際の配置結果で判定する
