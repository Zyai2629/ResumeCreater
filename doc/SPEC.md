...
> **「現在に至る」自動判定**: 最終職歴の内容に「退社」または「退職」が含まれない場合、「現在に至る」行が左寄せで自動追加される。最終職歴が退社・退職の場合は何も追加しない。履歴書には「以上」行を表示しない（職務経歴書のみ「以上」を表示）。
...
> **職歴ブロック間の余白**: 複数の職歴ブロックは隙間なく連続表示する（`margin-bottom: 0`）。

### 職務経歴書ページ溢れ処理

`Utils.adjustCareerOverflow(container)` がプレビュー表示時とPDF生成時に呼び出される。
...
- 続きページのタイトルは「■職務経歴（続き）」
- 移動後に新ページでも溢れる場合はさらに次ページを作成

#### ページ数警告
- 職務経歴書が**3ページ以上**になった場合、`Utils.showToast()` で警告を表示
- 警告メッセージ: 「職務経歴書が3ページ以上になっています。内容を簡潔にすることを検討してください。」

#### 資格セクションの同ページ統合
- 最終職歴ページの残余スペースが十分な場合、career-page2（資格・スキル・志望動機セクション）の内容を最終職歴ページに統合する
- 統合条件: `remainingSpace >= totalPage2Height + 5mm`
- 統合後、空になったcareer-page2は削除され、ページ番号が再計算される
- 余白が不足する場合は従来通りcareer-page2が独立ページとして表示される

#### ページ番号
- 全職務経歴書ページの`.page-number`要素を「N / M」形式に更新
- `position: absolute; bottom: 10mm; right: 11mm;`
...
- プレビュー・PDF上の写真枠（27mm × 35mm、`position: absolute; top: -13.1mm; left: 6mm;`）に表示
- 写真枠はテーブルセルの上端を超えてタイトル行の高さまで拡張（サンプル画像と完全一致）
- 未設定の場合は破線枠のみ表示
